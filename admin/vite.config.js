...
// to "dev" when git isn't available (e.g. some CI shallow contexts) so the
// __BUILD_SHA__/version `define` step below never fails on a missing SHA.
// NOTE: a production `pnpm build` still REQUIRES a git checkout — stampServiceWorker
// (below) intentionally THROWS on the "dev" fallback during `vite build` rather
// than emit a non-unique service-worker CACHE_NAME, so the "dev" fallback only
// keeps the define injection working (e.g. for `vite dev`), not the prod build.
// A fixed --short=12 length keeps the stamp deterministic as history grows (bare
// --short can change abbreviation length). execFileSync (no shell) with a fixed
// argument list — no interpolation, no injection surface.
let commitSha = 'dev'
try {
